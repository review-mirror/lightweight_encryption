...
#ifndef LIGHTWEIGHT_MD5_H
#define LIGHTWEIGHT_MD5_H
#include <memory.h>
#include <sstream>
#include <fstream>
#include <iomanip>
 ...
 * @return std::string
 */
std::string Md5SumString(unsigned char *dest_str, unsigned int dest_len)
{
  using namespace md5;
	...
	{
        ss << std::hex  << std::setw(2) << std::setfill('0') << (int)md5_value[i];
	}
 ...
 * @return md5_str
 */
std::string Md5SumFile(const char *file_path)
{
  using namespace md5;
	int i;
	...
	while (!ifs.eof())
	{
        ifs.read((char*)data, READ_DATA_SIZE);
        if (ifs.gcount() <= 0) {
            break;
        }

        MD5Update(&md5, data, ifs.gcount());
	}
  ifs.close();
